...
/// Write a Rust file to the build script output directory. The file is formatted if `rustfmt` is available.
///
/// To include the file outside the build script, use
///
/// ```ignore
/// include!(concat!(env!("OUT_DIR"), "/name_of_the_file.rs"));
/// ````
/// Where `name_of_the_file.rs` is the value of `out_file`.
...
/// # Panics
///
/// Panics if a valid identifier could not be formed.
