...
#[derive(Clone, Copy, Debug, Packet)]
#[packet(id = packet_id::COMMAND_TREE_S2C)]
pub(crate) struct RawCommandTreeS2c<'a>(pub &'a [u8]);

impl<'a> Encode for RawCommandTreeS2c<'a> {
...
pub enum NodeData<'a> {
    ...
    Argument {
        name: Cow<'a, str>,
        parser: Parser<'a>,
        suggestion: Option<NodeSuggestion>,
    },
}
        ...
        match &self.data {
            ...
            NodeData::Argument {
                ...
                if let Some(suggestion) = suggestion {
                    match suggestion {
                        NodeSuggestion::AskServer => "ask_server",
                        NodeSuggestion::AllRecipes => "all_recipes",
                        NodeSuggestion::AvailableSounds => "available_sounds",
                        NodeSuggestion::AvailableBiomes => "available_biomes",
                        NodeSuggestion::SummonableEntities => "summonable_entities",
                    }
                    .encode(&mut w)?;
                }
            }
            ...
            2 => NodeData::Argument {
                ...
                suggestion: if flags & 0x10 != 0 {
                    Some(match Ident::<Cow<str>>::decode(r)?.as_str() {
                        "minecraft:ask_server" => NodeSuggestion::AskServer,
                        "minecraft:all_recipes" => NodeSuggestion::AllRecipes,
                        "minecraft:available_sounds" => NodeSuggestion::AvailableSounds,
                        "minecraft:available_biomes" => NodeSuggestion::AvailableBiomes,
                        "minecraft:summonable_entities" => NodeSuggestion::SummonableEntities,
