...
use valence_core::translation_key::{
    ...
    ARGUMENT_INTEGER_LOW, ARGUMENT_LONG_BIG, ARGUMENT_LONG_LOW, ARGUMENT_POS_MIXED,
...
use crate::{parsing_error, parsing_ret_err, parsing_token};
...
impl<'a> Parsable<'a> for BlockStateArgument {
    ...
    fn parse(
        ...
        let mut state = BlockState::from_kind(match BlockKind::from_str(kind_str) {
            Some(kind) => kind,
            None => {
                return ParsingResult {
                    suggestions: Some((begin..reader.cursor(), BlockSuggestions::BlockKind)),
