...
use valence_command::parse::{parse_error_message, Parse};
...
fn command_listen(mut event: EventReader<CommandExecutionEvent>, mut client: Query<&mut Client>) {
    for exec in event.iter() {
        if exec.name() == "creative" {
            let mut client = client.get_mut(exec.client).unwrap();
