...
use valence::protocol::packets::s2c::play::{Commands, SoundCategory};
use valence::server::{Server, SharedServer, ShutdownResult};
use valence::text::{Color, TextFormat};
use valence::{async_trait, def_command, ident};
use vek::Vec3;
use valence::commands::CommandRegistry;
...
impl Config for Game {
    ...
    fn commands(&self) -> Commands {
        ...
        commands.register_command::<ComplexCommand>();
        let packet = commands.to_packet();
        println!("{:#?}", packet);
        packet
    }
